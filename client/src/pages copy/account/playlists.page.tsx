...
import { IPlaylist, PlaylistMode, debounce, useAction, usePlaylist, useTypedSelector } from '@shared';
...
const DynamicPlaylistModal = dynamic(() => import('../../widgets/PlaylistModal/PlaylistModal'), {
	loading: () => <p>Loading...</p>,
});

interface IPlaylistProps {}

export const debouncedFetchPl = debounce(() => store.dispatch(fetchUserPlaylists()), 500);

const AccountPlaylistsPage: FC<IPlaylistProps> = () => {
	//hooks
	const { userPlaylists, isUserPlaylistLoading, playlistsToDelete } = useTypedSelector(i => i.playlists);
	const { _playlist } = useAction();
	const dispatch = useAppDispatch();
	const deletePlaylistsOnUnmount = useCallback(() => {
		if (playlistsToDelete.length) dispatch(_playlist.deletePlaylists(playlistsToDelete));
	}, [playlistsToDelete]);
	...
	return (
		<AccountLayout>
			<ContentBlock header={Local.Account.MyPlaylists}>
				{isUserPlaylistLoading ? <PlaylistListSkeleton amount={10} /> : <AccountPlaylistList playlists={userPlaylists} />}
			</ContentBlock>
			{isVisible && (
...
export const AccountPlaylistList: FC<IAccountPlaylistsProps> = ({ playlists }) => {
