...
import { CloudUploadOutlined, KeyboardArrowLeft, KeyboardArrowRight } from '@material-ui/icons';
	...
	return (
		<>
			...
			<StepWrapper activeStep={activeStep}>
				...
				{activeStep === 1 && (
					<>
						<Grid container direction='column' justifyContent='center' alignItems='center' className='upload-icon'>
							<CloudUploadOutlined fontSize='inherit' color='inherit' />
							{picture ? (
								<Typography>{Local.CreateTrack.CoverIsUploaded}</Typography>
							) : (
								<Button>
									<FileUpload accept='image/*' setFile={uploadPicture} style={{ height: '100%' }}>
					...
					<>
						<Grid container direction='column' justifyContent='center' alignItems='center' className='upload-icon'>
							...
							{audio ? (
								...
								<Button>
									<FileUpload accept='audio/*' setFile={uploadAudio} style={{ height: '100%' }}>
			...
			{useIsMobile() ? (
				<MobileStepper
					...
					nextButton={
						<Button size='small' onClick={activeStep === 2 ? handleSendTrack : next}>
							{activeStep === 2 ? Local.CreateTrack.UploadTrack : Local.CreateTrack.Next}
							<KeyboardArrowRight />
						</Button>
					}
				/>
